...
const AddendumCard: React.FC<{ number: string; title: string; subtitle?: string; children: React.ReactNode }> = ({ number, title, subtitle, children }) => (
  <div className="border-b border-stone-200 py-12 last:border-b-0">
    <div className="flex flex-col md:flex-row gap-8">
      <div className="w-full md:w-1/4">
        <span className="text-stone-400 font-mono text-sm block mb-2">{number}</span>
        <h3 className="text-xl text-stone-900 font-medium">{title}</h3>
        {subtitle && <p className="text-stone-500 text-sm mt-1">{subtitle}</p>}
      </div>
      <div className="w-full md:w-3/4 space-y-4 text-stone-600 leading-relaxed">
        {children}
      </div>
    </div>
  </div>
...
export const Addendums: React.FC = () => {
  return (
    <section id="manifesto" className="py-24 px-6 md:px-24 bg-white scroll-mt-20 border-b border-stone-200">
      <h2 className="text-3xl text-stone-900 font-light mb-12 border-b border-stone-200 pb-6">Manifesto</h2>
      
      <AddendumCard number="01" title="Decision-First Architecture">
        <p className="text-stone-900 mb-2 font-medium">Most systems “support decisions.” Armonyco makes a stronger claim: nothing is valid until it has been decided.</p>
        <p>A Decision-First system does not optimize workflows. It governs reality.</p>
        <p className="mt-4"><strong className="text-stone-900">What this unlocks:</strong></p>
        ...
        <p className="mt-4 italic text-armonyco-gold font-medium">Armonyco’s output is not “automation.” It is verdicts + proof, at the point where execution becomes risk.</p>
      </AddendumCard>
      ...
      <AddendumCard number="03" title="Friendly Frontend. Adult Backend.">
        ...
        <p className="mt-4 italic text-armonyco-gold font-medium">Armonyco makes governance invisible—without making it weak.</p>
      </AddendumCard>
      ...
      <AddendumCard number="05" title="WhatsApp Is Not the Product." subtitle="It’s the “Dirty World Interface.”">
        ...
        <p className="mt-4 italic text-armonyco-gold font-medium">This is a huge point: Armonyco feeds on disorder—and becomes stronger because of it.</p>
      </AddendumCard>
      ...
      <AddendumCard number="07" title="Irreversibility" subtitle="(Structural Lock-In, Not Contractual)">
        ...
        <p className="mt-4 italic text-armonyco-gold font-medium">This is the power statement: Armonyco is designed to become indispensable, irreversible, and non-replaceable.</p>
      </AddendumCard>
      ...
      <div className="mt-24 p-12 border border-armonyco-gold/30 bg-stone-50 rounded-xl text-center shadow-sm">
        <h3 className="text-armonyco-gold uppercase tracking-widest text-sm mb-4">The Compass Line (Non-Negotiable Filter)</h3>
        <p className="text-2xl text-stone-900 font-light italic mb-6">"If Armonyco solves a problem that can be bypassed by a human, it isn’t Armonyco."</p>
